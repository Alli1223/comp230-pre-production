...
public class UniSelect : MonoBehaviour
{
    ...
    void Start()
    {
        tactCam = GetComponent<Camera>();
    }

    private void Update()
    {
        mouse = /*Switch.inst.GetTacticalCamera().*/tactCam.ScreenPointToRay(Input.mousePosition); //Get the mouse position from the tactical camera
    ...
    private void Raycast()
    {
        if (Input.GetMouseButtonUp(0)) //When left click...
        {
            ...
            {
                if (mouseClickHit.collider.gameObject.tag == "Player") //if you click on the player
                {
